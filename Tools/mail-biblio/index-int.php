<?php
	...
	include_once("./class/export-int.class.php");

	// Définition des paramètres
	$pathCairn				= PATHCAIRNINT;		// Adresse du site
	$pathStatic 			= PATHSTATICINT;	// Adresse de l'emplacement des éléments (images, css, ...)
	$pathVignette			= PATHVIGNETTEINT;	// Adresse d'accès aux vignettes
	$html 					= "";
	...
	$html_tpl_base 			= file_get_contents('tpl-int/tpl-base.html');
	$html_tpl_biblio_from	= file_get_contents('tpl-int/tpl-biblio-from.html');
	$html_tpl_commentaire	= file_get_contents('tpl-int/tpl-commentaire.html');
	$html_tpl_article 		= file_get_contents('tpl-int/tpl-article.html');
	$html_tpl_numero 		= file_get_contents('tpl-int/tpl-numero.html');
	$html_tpl_disclaimer	= file_get_contents('tpl-int/tpl-disclaimer.html');
	$html_tpl_inscription	= file_get_contents('tpl-int/tpl-inscription.html');
	...
	$html = str_replace("###TITRE###", "My selection", $html);
	...
	foreach($articles as $id_article) {
		...
		if(!empty($article)) {
			...
			foreach ($article[0] as $key => $value) {
				...
				$article_url = "http://www.cairn-int.info/abstract-".$id_article."--".$article[0]["ARTICLE_URL"].".htm?WT.mc_id=eBib";
				$tmp = str_replace("###ARTICLE_URL###", $article_url, $tmp);

				// Paramétrage de la vignette
				$article_vignette = $pathVignette.$article[0]["REVUE_ID_REVUE"]."/".$article[0]["NUMERO_ID_NUMERO"]."_L62.jpg";
		...
		if(!empty($numero)) {
			...
			foreach ($numero[0] as $key => $value) {
				...
				// Revue
				if(($typepub == 1)) {
					$numero_url = "http://www.cairn-int.info/journal-".$numero[0]["REVUE_URL"]."-".$numero[0]["NUMERO_ANNEE"]."-".$numero[0]["NUMERO_NUMERO"].".htm?WT.mc_id=eBib";
					$tmp = str_replace("###NUMERO_URL###", $numero_url, $tmp);
				}
				
				// Paramétrage de la vignette
				$article_vignette = $pathVignette.$numero[0]["REVUE_ID_REVUE"]."/".$numero[0]["NUMERO_ID_NUMERO"]."_L62.jpg";
	...
	$to 		= $to_email;
	$subject 	= "My selection on Cairn.info";
	$message 	= $html;
